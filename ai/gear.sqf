...
if (_this == (missionNamespace getVariable 'objective_owner') && 'scorch_invitems' call caran_checkMod) then {
	_items set [count _items, ['sc_harddrive', 1, 'Uniform']];
};
...
switch _gang do {
	case 'shemags': {
		_uniform = _uniform_shemags select floor random count _uniform_shemags;
	...
	case 'military': {
		_uniform = _uniform_military select floor random count _uniform_military;
	...
	case 'sandals': {
		_uniform = _uniform_sandals select floor random count _uniform_sandals;
	...
	case 'terrors': {
		_uniform = _uniform_terrors select floor random count _uniform_terrors;
