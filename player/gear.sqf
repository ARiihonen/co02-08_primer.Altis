...
if ( "acre_" call caran_checkMod ) then {
	...
	if (player == leader group player || _class == 'B_SOLDIER_TL_F' ) then {
		_items set [count _items, ["ACRE_PRC148", 1, "Vest"]];
	};
} else {
...
if ( 'ace_' call caran_checkMod ) then {
	_items set [ count _items, ['ACE_elasticBandage', 6, 'Uniform'] ];
